...
if (isset($_GET['id']) && $_GET['id'] != '') {
    ...
    $res = mysqli_query($conn, "select * from size_master where id='$id'");
...
if (isset($_POST['submit'])) {
    ...
    $res = mysqli_query($conn, "select * from size_master where size='$size'");
    $check = mysqli_num_rows($res);
    if ($check > 0) {
        if (isset($_GET['id']) && $_GET['id'] != '') {
    ...
    if ($msg == '') {
        if (isset($_GET['id']) && $_GET['id'] != '') {
            mysqli_query($conn, "update size_master set size='$size',order_by='$order_by' where id='$id'");
        } else {
